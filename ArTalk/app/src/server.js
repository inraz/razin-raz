...
/**
 * arTalk P2P - Server component
 *
 * @link    GitHub: https://github.com/------/artalk
 * @link    Official Live demo: https://p2p.artalk.com
 * @license For open source use: AGPLv3
 * @license For commercial use or closed source, contact us at license.artalk@gmail.com or purchase directly from CodeCanyon
 * @license CodeCanyon: https://codecanyon.net/item/artalk-p2p-webrtc-realtime-video-conferences/38376661
 * @author  ar edits - arcreations@gmail.com
 * @version 1.0.5
 *
 */
...
const api_key_secret = process.env.API_KEY_SECRET || 'artalk_default_secret';
...
app.get('/join/', (req, res) => {
    if (hostCfg.authenticated && Object.keys(req.query).length > 0) {
        log.debug('Request Query', req.query);
        /* 
            http://localhost:3000/join?room=test&name=artalk&audio=1&video=1&screen=1&notify=1
            https://p2p.artalk.com/join?room=test&name=artalk&audio=1&video=1&screen=1&notify=1
            https://artalk.up.railway.app/join?room=test&name=artalk&audio=1&video=1&screen=1&notify=1
            https://artalk.herokuapp.com/join?room=test&name=artalk&audio=1&video=1&screen=1&notify=1
        */
...
app.post([apiBasePath + '/meeting'], (req, res) => {
    ...
    if (authorization != api_key_secret) {
        log.debug('arTalk get meeting - Unauthorized', {
            headers: headers,
            body: body,
        });
    ...
    log.debug('arTalk get meeting - Authorized', {
        headers: headers,
        body: body,
        meeting: meetingURL,
    });
